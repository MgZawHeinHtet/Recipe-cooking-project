  ...
  <div>
    ...
    <section class="px-32 mb-16">
      ...
      <div v-if="meals"  class="grid grid-cols-5 w-full gap-3">
        <div v-for="(meal,index) in meals" :key="index" class="rounded">
          <div class="menu_card rounded space-y-3">
            <div class="menu_img h-64 w-full bg-[url('../assets/nav_logo.png')] bg-center bg-contain bg-no-repeat rounded-lg">
              <img v-show="imgLoading" @load="showImg"  class="object-cover h-full rounded"   :src="meal.strMealThumb" alt />
            </div>
      ...
      <div v-else class="w-full h-40 rounded-lg flex justify-center items-center">
        <p v-if="loading"><i class="fa-solid fa-spinner animate-spin text-5xl"></i></p>
        <p v-else-if="errMsg" class="text-orange-700 uppercase text-base font-semibold tracking-wider">💥{{errMsg}}</p>
        <p v-else class="text-slate-700 uppercase text-xl font-semibold tracking-wider">Welcome bro,You didn't search anything 😁</p>
      </div>
    </section>
  </div>
</template>
